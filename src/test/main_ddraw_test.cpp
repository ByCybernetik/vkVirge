...
// Windowed DirectDraw7 test used with the wrapper
...
static HRESULT InitDirectDraw(HWND hwnd) {
    ...
    hr = IDirectDraw7_SetCooperativeLevel(g_pDD, hwnd, DDSCL_NORMAL);
    if (FAILED(hr)) return hr;

    // Windowed primary surface
    DDSURFACEDESC2 ddsd = {};
    ...
    hr = IDirectDraw7_CreateSurface(g_pDD, &ddsd, &primary, nullptr);
    if (FAILED(hr)) return hr;
    g_pPrimarySurface = primary;

    // Simple offscreen back buffer in system memory
    ZeroMemory(&ddsd, sizeof(ddsd));
    ddsd.dwSize  = sizeof(ddsd);
    ddsd.dwFlags = DDSD_CAPS | DDSD_WIDTH | DDSD_HEIGHT | DDSD_PIXELFORMAT;
    ddsd.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_SYSTEMMEMORY;
    ddsd.dwWidth  = g_dwScreenWidth;
    ddsd.dwHeight = g_dwScreenHeight;
    ...
    hr = IDirectDraw7_CreateSurface(g_pDD, &ddsd, &g_pBackSurface, nullptr);
    return hr;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow) {
    ...
    wc.lpszClassName = L"DDVKWindowedTest";
    ...
    RECT rc = { 0, 0, (LONG)g_dwScreenWidth, (LONG)g_dwScreenHeight };
    AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW, FALSE);

    HWND hwnd = CreateWindowEx(
        0,
        L"DDVKWindowedTest",
        L"DDVK Windowed DDraw Test",
        WS_OVERLAPPEDWINDOW,
    ...
    while (running) {
        ...
        if (g_bActive && g_pPrimarySurface && g_pBackSurface) {
            DrawTestPattern(g_pBackSurface, RGB(0, 128, 0));

            RECT src = { 0, 0, (LONG)g_dwScreenWidth, (LONG)g_dwScreenHeight };
            RECT dst;
            GetClientRect(g_hWnd, &dst);

            IDirectDrawSurface7_Blt(g_pPrimarySurface, &dst, g_pBackSurface, &src,
                                    DDBLT_WAIT, nullptr);
        }
